...
- 测试方式：首版优先使用手机号验证码登录；微信登录需要说明开放平台配置状态
- 内部监控：上线准备度和运行监控默认不展示给普通用户，仅开发开关可见

## 登录和账号

- 手机号验证码登录已恢复为首版默认入口；上线前必须配置真实短信服务商并完成到达率测试。
- 微信登录按钮已保留，并已接入后端换码接口和 iOS 状态检测；正式可用前需要完成微信开放平台移动应用配置和 iOS OpenSDK 接入。
- Apple 登录已接入后端验签和 iOS 原生桥接，但当前默认不展示；如后续启用，需要在 Apple Developer 和 Xcode target 开启 `Sign in with Apple`。
...
- 已声明的数据类型：
  - 手机号、短信验证码登录记录、微信 openid 或 Apple 登录用户标识：用于登录和账户管理
  - 用户 ID：用于账户同步
  - 持仓和交易输入：用于持仓分析和风险观察
...
测试方式：
1. 打开 App 后使用手机号验证码登录；如微信登录审核完成，也可以测试微信登录。
