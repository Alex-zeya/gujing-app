...
股镜首版登录入口保留手机号验证码。微信登录作为国内用户的重点入口继续推进，开放平台移动应用审核已经通过，AppID 和 AppSecret 可以配置到线上后端。Apple 登录能力保留，但当前默认后置。

微信登录当前处于“开放平台审核已通过 / iOS OpenSDK 待接入”阶段。前后端已经预留：
...
还需要在 Xcode 工程中接入微信 OpenSDK，并让 `GujingWechatLogin` 原生插件真正调用微信授权。当前前端按钮和后端换码已经保留，缺的是微信官方 SDK 的真机授权链路。

接入完成后需要验证：

- 点击微信登录能唤起微信。
- 授权后能回到股镜。
- 后端 `/api/auth/wechat/login` 能用授权 code 换到 openid。
- App 内能拿到股镜自己的登录 token。
- 退出登录后再次微信登录不会创建重复账户。
